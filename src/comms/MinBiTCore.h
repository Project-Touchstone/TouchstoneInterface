...
class MinBiTCore {
    public:
        ...
        MinBiTCore(std::shared_ptr<IStream> stream);
        ~MinBiTCore();
        ...
        bool getExpectedPacketLength(uint8_t header, std::size_t length) const;
    ...
    private:
        ...
        std::unordered_map<uint8_t, std::size_t> packetLengths;
